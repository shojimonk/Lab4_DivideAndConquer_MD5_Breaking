...
int main(void)
{
	...
	/*
	struct workerDataSend *toSend = newDataSender();

	//toSend = (struct WorkerDataSend*)malloc(sizeof(struct WorkerDataSend));
	
	toSend->numThreads = slaveThreads;
	toSend->wordLength = goalLength;
	strcpy(toSend->hash, theHash);
	*/
